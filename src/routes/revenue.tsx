...
import { revenueSeries, platformMetrics } from "@/lib/mock-data";
...
function RevenuePage() {
  const arr = platformMetrics.monthlyRevenue * 12;
  return (
    <div className="space-y-6">
      <Topbar title="Revenue" subtitle="SaaS revenue intelligence and projections" />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="MRR" value={platformMetrics.monthlyRevenue} prefix="₹" delta={14.7} icon={IndianRupee} accent="emerald" />
        <StatCard label="ARR (projected)" value={arr} prefix="₹" delta={18.2} icon={TrendingUp} accent="cyan" delay={0.05} />
        <StatCard label="Net retention" value={108.4} suffix="%" decimals={1} delta={2.1} icon={Repeat} accent="violet" delay={0.1} />
        <StatCard label="Avg revenue per landlord" value={Math.round(platformMetrics.monthlyRevenue / platformMetrics.activeLandlords)} prefix="₹" delta={4.6} icon={Wallet} accent="blue" delay={0.15} />
      </div>
