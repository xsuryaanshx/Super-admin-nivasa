...
import {
  platformMetrics, revenueSeries, activityFeed, landlords, systemStatus,
} from "@/lib/mock-data";
...
function Overview() {
  const recentLandlords = [...landlords].sort((a, b) => b.joinedAt.localeCompare(a.joinedAt)).slice(0, 5);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Active Landlords" value={platformMetrics.activeLandlords + 1256} delta={12.4} icon={Users} accent="cyan" delay={0.0} />
        <StatCard label="Total Buildings" value={platformMetrics.totalBuildings} delta={8.2} icon={Building2} accent="violet" delay={0.05} />
        <StatCard label="Total Properties" value={platformMetrics.totalProperties} delta={5.1} icon={Home} accent="blue" delay={0.1} />
        <StatCard label="Total Rooms" value={platformMetrics.totalRooms} delta={6.8} icon={BedDouble} accent="cyan" delay={0.15} />
        <StatCard label="Total Tenants" value={platformMetrics.totalTenants} delta={9.4} icon={UsersRound} accent="emerald" delay={0.2} />
        <StatCard label="Monthly SaaS Revenue" value={platformMetrics.monthlyRevenue} prefix="₹" delta={14.7} icon={IndianRupee} accent="emerald" delay={0.25} />
        <StatCard label="Active Subscriptions" value={platformMetrics.activeSubscriptions} delta={3.6} icon={CreditCard} accent="violet" delay={0.3} />
        <StatCard label="Churn Rate" value={platformMetrics.churnRate} suffix="%" decimals={1} delta={-0.4} icon={TrendingDown} accent="blue" delay={0.35} />
      </div>
              ...
              return (
                ...
                >
                  <div className="h-8 w-8 rounded-lg grid place-items-center"
                       style={{ background: "oklch(1 0 0 / 4%)", color: activityColor[e.type] }}>
